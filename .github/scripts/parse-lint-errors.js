...
function parseMarkdownReport(mdPath) {
  try {
    ...
    const filePattern = /##?\s+(.+?\.(tsx?|css|jsx?|json))/g;
    const errorPattern = /(?:-|\*|\d+\.)\s*(.+?)(?:\n|$)/g;
    const linePattern = /(?:line|行)\s*(\d+)/i;
    const severityPattern = /(error|warning|warn|info|エラー|警告|情報)/i;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // エラーメッセージの検出
      if (currentFile && (line.includes('error') || line.includes('warning') || line.includes('エラー') || line.includes('警告'))) {
        ...
        errors.push({
          ...
          severity: severityMatch ? severityMatch[1].toLowerCase() : 'error',
          message: line.replace(/^[-*\d.\s]+/, '').trim(),
          rule: extractRuleName(line),
        });
      }
    }
    ...
    while ((match = detailedPattern.exec(content)) !== null) {
      ...
      // エラーメッセージを抽出
      const messageMatch = context.match(/(?:error|warning|エラー|警告)[:\s]+(.+?)(?:\n|$)/i);
      ...
      errors.push({
        ...
        message: messageMatch ? messageMatch[1].trim() : 'エラーが検出されました',
